...
The current Antigravity quota UI exposes rows such as "Gemini 3.5 Flash (Low)" and "Gemini 3.5 Flash (High)". OpenCode only treats configured model keys as selectable model IDs in all contexts, including scripts and agent configs. Direct IDs therefore avoid "Model not found" errors in non-interactive usage.
...
- `antigravity-gemini-3.5-flash-high` → API model `gemini-3-flash-agent` + `thinkingLevel: "high"`
