...
- **CliTransport — opt-in `agy` agent backend (Phase 4, experimental):**
  ...
  - Maps stdout to Gemini text response; maps timeout/auth/process errors to 504/401/502.
  - Does **not** forward OpenCode tool calls to `agy` (double-agent by design).
  - Added dedicated tests for `agy` process handling and CLI transport behavior.
...
- **ManagedAgentTransport — opt-in public Interactions API (Phase 5, experimental):**
  ...
  - Uses `antigravity-preview-05-2026` base agent, `Api-Revision: 2026-05-20`.
  - Disabled by default (`transport.managed_agent.enabled: false`).
  - Supports multi-turn via `previous_interaction_id`, `system_instruction`, `environment`.
  - Added dedicated tests for request construction, response mapping, multi-turn fields, and streaming passthrough.
...
- **New docs:**
  - `docs/CONFIGURATION.md` — transport config, auth modes, environment variables.
  - `docs/SUPPORT_MATRIX.md` — feature/model/credential/rate-limit matrix per transport.
  - `docs/ANTIGRAVITY_CLI_MIGRATION_PLAN.md` — full phased migration roadmap (Phases 0–6).
  - `docs/ANTIGRAVITY_PHASE0_BASELINE.md` — evidence snapshot before migration.

### Changed

- **Primary endpoint** changed from `daily-cloudcode-pa.sandbox.googleapis.com` to `daily-cloudcode-pa.googleapis.com` (matches official `agy` CLI behavior).
- **Default model list** no longer includes Gemini CLI models because individual Gemini CLI access sunsets on 2026-06-18. Legacy resolver/fallback support remains for existing configs.
- **Debug logging** now includes `transport=`, `action=`, `accountIdx=`, and `hasEmail=` fields per outbound request.
- **`verifyAccountAccess`**, **`fetchAvailableModels`**, and **`fetchGeminiCliQuota`** documented as prod-only by design.
- `docs/ARCHITECTURE.md` updated with transport boundary section, module structure, and responsibility table.
- `docs/TROUBLESHOOTING.md` updated with OAuth, transport, and billing troubleshooting sections.
- `docs/ANTIGRAVITY_API_SPEC.md` updated with endpoint status table and `openid` scope.

### Risk Notes

- **Gemini CLI sunset: 2026-06-18.** The `gemini-cli` header style quota pool will stop serving requests for Google AI Pro/Ultra and free Gemini Code Assist individuals after this date. If requests fail after June 18, disable `quota_fallback` and rely on the `antigravity` header style only.
- **CliTransport and ManagedAgentTransport are experimental.** Both are disabled by default. Do not enable them in production without testing.
